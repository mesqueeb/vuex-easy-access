...
function defaultSetter(path, payload, store) {
  ...
  if (conf.vuexEasyFirestore && firestoreConf) {
    ...
    var newPayload = !props ? payload : createObjectFromPath(props, payload, _module.state, conf);
...
var friendsList = {
  namespaced: true,
  state: state$1,
  mutations: defaultMutations(state$1, config, { moduleNamespace: 'friendsList/' })
};

// set plugins
var easyAccess = createEasyAccess(config);
var easyFirestores = createFirestores([firestoreDex, firestoreUser]);
...
// export store
var storeObj = {
  modules: { locationJournal: locationJournal, user: user, dex: dex, friendsList: friendsList },
  state: initialState(),
